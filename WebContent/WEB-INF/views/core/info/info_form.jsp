  ...
  <c:when test="${field.name eq 'attributes'}">
    ...
    <tr id="attr_img_${attr.id}"<c:if test="${!fnx:contains_co(attrs,attr)}"> style="display:none;"</c:if>>
    	...
    	<td class="in-ctt" width="85%" colspan="3">
    		<tags:image_upload name="attrImages_${attr.id}" value="${fnx:invoke1(bean,'getInfoAttr',attr).image}" scale="${attr.scale}" exact="${attr.exact}" watermark="${attr.watermark}" width="${attr.imageWidth}" height="${attr.imageHeight}"/>
    </c:if>
    </c:forEach>
  </c:when>
