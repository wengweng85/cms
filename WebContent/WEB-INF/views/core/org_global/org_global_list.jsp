...
<div class="c-bar margin-top5">
  <span class="c-position"><s:message code="orgGlobal.management"/> - <s:message code="list"/></span>
...
<form action="list.do" method="get">
    ...
    <legend><s:message code="search"/></legend>
	  <label class="c-lab"><s:message code="org.name"/>: <input type="text" name="search_CONTAIN_name" value="${search_CONTAIN_name[0]}" style="width:120px;"/></label>
	  <label class="c-lab"><s:message code="org.number"/>: <input type="text" name="search_CONTAIN_number" value="${search_CONTAIN_number[0]}" style="width:120px;"/></label>
	  <label class="c-lab"><s:message code="org.parent"/>: 
	  	<select name="queryParentId" onchange="this.form.submit();">
        <option value=""></option>
	  ...
	  <label class="c-lab"><s:message code="org.showDescendants"/>: <f:checkbox name="showDescendants" value="${showDescendants}" default="true" onclick="this.form.submit();"/></label>
...
<div class="ls-bc-opt">
	...
	<div class="ls-btn"><input type="button" value="<s:message code="create"/>" onclick="location.href='create.do?showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}';"/></div>
...
<table id="pagedTable" border="0" cellpadding="0" cellspacing="0" class="ls-tb margin-top5">
  ...
  <tr beanid="${bean.id}">
    ...
    <td align="center">
			<shiro:hasPermission name="core:org_global:copy">
      <a id="copy_opt_${bean.id}" href="create.do?id=${bean.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}" class="ls-opt"><s:message code="copy"/></a>
      </shiro:hasPermission>
			<shiro:hasPermission name="core:org_global:edit">
      <a id="edit_opt_${bean.id}" href="edit.do?id=${bean.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&position=${pagedList.number*pagedList.size+status.index}&${searchstring}" class="ls-opt"><s:message code="edit"/></a>
      </shiro:hasPermission>
			<shiro:hasPermission name="core:org_global:delete">
      <a href="delete.do?ids=${bean.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}" onclick="return confirmDelete();" class="ls-opt"><s:message code="delete"/></a>
      </shiro:hasPermission>
     </td>
    <td><c:out value="${bean.id}"/><f:hidden name="id" value="${bean.id}"/></td>
    <td><c:if test="${showDescendants}"><span style="padding-left:${bean.treeLevel*12}px"></c:if><f:text name="name" value="${bean.name}" style="width:120px;"/></span></td>
    <td align="center"><f:text name="number" value="${bean.number}" style="width:100px;"/></td>
    <td align="center"><f:text name="phone" value="${bean.phone}" style="width:100px;"/></td>
    <td align="center"><f:text name="address" value="${bean.address}" style="width:120px;"/></td>
  </tr>
  </c:forEach>
  </tbody>
</table>
