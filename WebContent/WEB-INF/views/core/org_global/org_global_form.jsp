...
<div class="c-bar margin-top5">
  <span class="c-position"><s:message code="orgGlobal.management"/> - <s:message code="${oprt=='edit' ? 'edit' : 'create'}"/></span>
</div>
...
<table border="0" cellpadding="0" cellspacing="0" class="in-tb margin-top5">
  <tr>
    <td colspan="4" class="in-opt">
			...
			<div class="in-btn"><input type="button" value="<s:message code="create"/>" onclick="location.href='create.do?parentId=${parent.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}';"<c:if test="${oprt=='create'}"> disabled="disabled"</c:if>/></div>
			<div class="in-btn"></div>
			</shiro:hasPermission>
			<shiro:hasPermission name="core:org_global:copy">
			<div class="in-btn"><input type="button" value="<s:message code="copy"/>" onclick="location.href='create.do?id=${bean.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}';"<c:if test="${oprt=='create'}"> disabled="disabled"</c:if>/></div>
			</shiro:hasPermission>
			<shiro:hasPermission name="core:org_global:delete">
			<div class="in-btn"><input type="button" value="<s:message code="delete"/>" onclick="if(confirmDelete()){location.href='delete.do?ids=${bean.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}';}"<c:if test="${oprt=='create'}"> disabled="disabled"</c:if>/></div>
			</shiro:hasPermission>
			<div class="in-btn"></div>
			<div class="in-btn"><input type="button" value="<s:message code="prev"/>" onclick="location.href='edit.do?id=${side.prev.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&position=${position-1}&${searchstring}';"<c:if test="${empty side.prev}"> disabled="disabled"</c:if>/></div>
			<div class="in-btn"><input type="button" value="<s:message code="next"/>" onclick="location.href='edit.do?id=${side.next.id}&showDescendants=${showDescendants}&queryParentId=${queryParentId}&position=${position+1}&${searchstring}';"<c:if test="${empty side.next}"> disabled="disabled"</c:if>/></div>
			<div class="in-btn"></div>
			<div class="in-btn"><input type="button" value="<s:message code="return"/>" onclick="location.href='list.do?showDescendants=${showDescendants}&queryParentId=${queryParentId}&${searchstring}';"/></div>
  ...
  <tr>
    ...
    <f:text id="parentIdName" value="${parentName}" readonly="readonly" style="width:300px;"/><input id="parentIdButton" type="button" value="<s:message code='choose'/>"/>
    <script type="text/javascript">
    $(function(){
  ...
  <tr>
    <td class="in-lab" width="15%"><s:message code="org.contacts"/>:</td>
    <td class="in-ctt" colspan="3"><f:text name="contacts" value="${oprt=='edit' ? bean.contacts : ''}" maxlength="100" style="width:180px;"/></td>
  </tr>
  <tr>
    <td class="in-lab" width="15%"><s:message code="org.fullName"/>:</td>
    <td class="in-ctt" colspan="3"><f:text name="fullName" value="${oprt=='edit' ? bean.fullName : ''}" maxlength="100" style="width:500px;"/></td>
  </tr>
  <tr>
