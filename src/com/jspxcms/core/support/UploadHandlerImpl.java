...
import com.jspxcms.common.swf.OpenOfficeConverter;
import com.jspxcms.common.swf.SwfConverter;
...
public class UploadHandlerImpl implements UploadHandler {
	protected final Logger logger = LoggerFactory
			.getLogger(UploadHandlerImpl.class);
	...
	public void upload(String url, String type, Site site, Integer userId,
		...
		upload(url, type, site, userId, ip, result, null, null, null, null,
				null, null, null, null);
	}

	public void upload(MultipartFile partFile, String type, Site site,
			Integer userId, String ip, UploadResult result) {
		upload(partFile, type, site, userId, ip, result, null, null, null,
	...
	public void upload(String url, String type, Site site, Integer userId,
		...
		try {
			...
			try {
				doUpload(temp, fileName, type, site, userId, ip, result, scale,
	...
	private UploadResult doUpload(File file, String fileName, String type,
			Site site, Integer userId, String ip, UploadResult result,
		...
		if (Uploader.IMAGE.equals(type)) {
			...
			if (!"swf".equals(ext)) {
				...
				doUploadDoc(fileHandler, pathname, ext, pdfPathname,
	...
	private void doUploadDoc(FileHandler fileHandler, String pathname,
			String extension, String pdfPathname, String swfPathname,
			File file, String ip, Integer userId, Integer siteId)
			throws Exception {
		File pdfFrom = file;
		File pdfTemp = null;
		if (StringUtils.isNotBlank(pdfPathname)) {
			pdfTemp = File.createTempFile("pdf", ".pdf");
			OpenOfficeConverter.doc2pdf(file, pdfTemp,
					Constants.OPENOFFICE_HOST, Constants.OPENOFFICE_PORT);
			pdfFrom = pdfTemp;
		}
		...
		attachmentService.save(swfPathname, swfTemp.length(), ip, userId,
				siteId);
		if (pdfTemp != null) {
			attachmentService.save(pdfPathname, swfTemp.length(), ip, userId,
					siteId);
		}
		for (File f : files) {
