...
public class FInfo extends EntityPathBase<Document> {
	...
	public static Document doc(Info info) {
		...
		if (CollectionUtils.isNotEmpty(infoTags)) {
			...
			for (InfoTag infoTag : infoTags) {
				...
				doc.add(new Field(KEYWORD, tagName, NO, NOT_ANALYZED));
			}
		}
		...
		try {
			...
			if (StringUtils.isNotBlank(q)) {
				...
				Query qy = MultiFieldQueryParser.parse(LUCENE_36, q,
						new String[] { TITLE, TEXT }, new Occur[] { SHOULD,
								SHOULD }, analyzer);
				query.add(qy, MUST);
			}
			if (StringUtils.isNotBlank(title)) {
	...
	public static Predicate query(Integer[] siteIds, Integer[] nodeIds,
		...
		if (StringUtils.isNotBlank(q)) {
			exp = exp.and(info.title.contains(q).or(info.text.contains(q)));
		}
		if (StringUtils.isNotBlank(title)) {
			exp = exp.and(info.title.contains(title));
		}
		if (ArrayUtils.isNotEmpty(keywords)) {
			exp = exp.and(info.keyword.in(keywords));
		}
		if (StringUtils.isNotBlank(description)) {
